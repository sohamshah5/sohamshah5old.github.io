    ...
    return declare([BaseWidget, dijitWidgetBase, dijitWidgetsInTemplate], {
      ...
      postCreate: function () {
        ...
        if (this.config.hasOwnProperty("threatTypes")) {
          ...
          this._threatData = this._convertDistanceMetersToFeet();
        } else {
          //Retrieve threat types
          this._threatData = dojoJSON.parse(threats);
        }
        ...
        this._populateSelectList(this.layerSelect, this._layerList, '', true, false);

        // populate the publish list
        this._populateSelectList(this.featureLayerList, this._layerList,
          this.config.threatAnalysis.operationalLayer.name, false, true);
      ...
      _threatPublishButtonClicked: function () {
        ...
        var layerName = (this.publishNewLayer.checked) ? this.addTANameArea.value :
      ...
      _onLayerSelectionChanged: function () {
        var selectedLayerName = this.layerSelect.get("value");
