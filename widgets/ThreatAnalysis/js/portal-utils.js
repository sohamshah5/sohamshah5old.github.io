        ...
        if (response0.available) {
                ...
                function (response2) {
                  if (response2.success) {
                    ...
                    var newURL = '<br /><a role="link" tabindex="0" aria-label="' +
                      ...
                      "home/item.html?id=" + response1.itemId + '" target="_blank">' + nls.manageWebLayerText + '</a>';
                    publishMessage.innerHTML = nls.successfullyPublished + newURL;
          ...
          if (selectedLayer) {
            ...
            var newURL = '<br /><a role="link" tabindex="0" aria-label="' +
              ...
              nls.manageWebLayerText + '</a>';
            publishMessage.innerHTML =
              nls.successfullyAppended + newURL;
            topic.publish("setLastFocusNode", true);
          } else {
            topic.publish("setBusyIndicator", false);
            publishMessage.innerHTML = nls.layerNameExists;
            topic.publish("setLastFocusNode", false);
          }
        }
